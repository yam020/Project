...
class Data: 
	def __init__(self):
		...
		self.url = "https://raw.githubusercontent.com/nychealth/coronavirus-data/master/trends/"

	def load_data(self, string):
		if string == "casert":
			url_string = f"{self.url}caserate-by-modzcta.csv"
			self.df = pd.read_csv(url_string, header=None)
		if string == "percpos":
			url_string = f"{self.url}percentpositive-by-modzcta.csv"
			self.df = pd.read_csv(url_string, header=None)
		return self.df

class Trend:
	# write different functions to generate different trend graphs 
	def trendgraph(self, case, zipcode):
		return ... # graph object
...
class Map:
	...
	def clean_data(self):
		...
		df_percpos_clean = self.data.load_data("percpos").copy()
	...
	def map(self, string):
		if string == "percpos":
			# data cleanup for percentage postive 
			df_percpos_clean = self.clean_data()
