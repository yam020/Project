...
class Data:
	...
	def get_clean_data_percpos(self):
	...
	def get_clean_data_caserate(self):
		df_clean = self.df.copy()
		columnname = df_clean[:1].values.flatten().tolist()
		...
		date = df_clean.iloc[0,0]
		df_clean = df_clean.iloc[1:].reset_index()
		df_clean.columns = ["modzcta", "casert"]
		df_clean["casert"] = pd.to_numeric(df_clean["casert"], errors='ignore')
		return df_clean
